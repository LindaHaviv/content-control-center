...
# from the repo root
fly launch --no-deploy --copy-config --name second-brain-mcp   # uses deploy/fly.toml
...
fly deploy
```
Your server is then at `https://second-brain-mcp.fly.dev` (health: `/health`, MCP: `/mcp`).
...
## Connect clients
- **Claude Code / Claude Desktop / the API:** point them at `https://second-brain-mcp.fly.dev/mcp`
